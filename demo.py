...
from constants import API_VERSION, SAMPLE_USERS
...
def demo_request_response_examples():
    ...
    sample_user_list = [
        {"id": idx + 1, **user} for idx, user in enumerate(SAMPLE_USERS[:2])
    ]
    ...
    get_response = {
        "error": False,
        "data": {
            "users": sample_user_list,
            "total": len(sample_user_list),
            "page": 1,
            "per_page": 5,
            "pages": 1,
        },
...
def run_complete_demo():
    ...
    print(f"\nTo run the actual API (v{API_VERSION}):")
